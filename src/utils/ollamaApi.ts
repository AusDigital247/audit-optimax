...
/**
 * Function to generate AI response using OLLAMA API
...
export const generateOllamaResponse = async (prompt: string): Promise<string> => {
  try {
    ...
    // For development and testing, if OLLAMA is not available, use a fallback response
    if (process.env.NODE_ENV === 'development' || !OLLAMA_API_URL) {
      console.log('Using fallback response for development');
      // Simulate API delay
      await new Promise(resolve => setTimeout(resolve, 1000));
      return `This is a fallback response for development purposes. In production, this would be generated by the OLLAMA API using the prompt: "${prompt}"`;
    }
...
/**
 * Rewrites a paragraph using OLLAMA API
...
/**
 * Rewrites a sentence using OLLAMA API
...
/**
 * Generates blog ideas using OLLAMA API
